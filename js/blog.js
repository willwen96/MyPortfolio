...
// export { add, editHandler, deleteHandler, addPost, cancelPost };
...
function updatePosts() {
    ...
    for (let i = 0; i < posts.length; i++) {
        let post = posts[i];
        let text = `${post.title} ${post.date} ${post.summary}`;

        list += "<li>";
        list += text + " ";
        list += `<button onClick="editListener(${i})">Edit</button>`
        list += `<button class="delete-button" onclick="deleteListener(${i})">Delete</button>`
        list += "</li>"
    }
